...
import { DepartmentSubcategory, Subcategory } from '@/types';
...
export const SubcategorySelect = ({
  ...
  currentSubcategories: DepartmentSubcategory[];
  projectId: string;
  departmentId: string;
  onSubcategoryChange?: (subcategories: DepartmentSubcategory[]) => void;
  refreshProject: () => void;
}) => {
  ...
  const [selectedSubcategories, setSelectedSubcategories] =
    useState<DepartmentSubcategory[]>(currentSubcategories);
  ...
  const handleSubcategoryClick = (subcategory: DepartmentSubcategory) => {
    ...
    //   (selected) => selected.subcategory.id === subcategory.subcategory.id
    // );
              ...
              {allSubcategories.map((subcategory) => {
                const isSelected = selectedSubcategories.some(
                  (selected) =>
                    ...
                    selected.subcategory.id === subcategory.subcategory.id
                );
                return (
                  <div
                    key={subcategory.subcategory.id}
